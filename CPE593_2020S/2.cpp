...
void MUL(int** MatrixA, int** MatrixB, int** MatrixResult, int lMatrixSize);
void initialMatrix(int** matrix1, int** matrix2, int MatrixSize);
int main()
{
    ofstream fout("matrix_result.dat");
    fout << "#N\t\t"  <<"runtime_square\t\t" << "runtime_strassen\t\t" << "bound_square\t\t" << "bound_strassen\t\t" <<  endl;
    //cout << setprecision(2) << "#N\t\t" << "runtime_square_matrix\t\t" << "runtime_strassen_method\t\t" << "bound_square_matrix\t\t" << "bound_strassen_method\t\t" << endl;
    ...
    for (MatrixSize = 2; MatrixSize < 2048; MatrixSize = MatrixSize * 2) {
        ...
        initialMatrix(MatrixA, MatrixB, MatrixSize);
        ...
        float time_1 = ((endTime_For_Normal_Multipilication - startTime_For_Normal_Multipilication)/ (double)CLOCKS_PER_SEC)*1000000;
        float time_2 = ((endTime_For_Strassen - startTime_For_Strassen)/ (double)CLOCKS_PER_SEC)*1000000;
        float c ;
        float d ;
        double f = pow(MatrixSize, 2);
        double e = pow(MatrixSize, 1.81);
        //a += time_1;
        //b += time_2;
        c = (9128000 / 2046) * f;
        ...
        fout << MatrixSize<<"\t\t"<< time_1 << "\t\t"<< time_2  << "\t\t" << c << "\t\t" << d << "\t\t" << endl;
        //cout << MatrixSize << "\t\t" << time_1 << "\t\t" << time_2 << "\t\t" << d << "\t\t" << d << "\t\t" << endl;
        //cout << a << endl;
        //fout << (endTime_For_Normal_Multipilication - startTime_For_Normal_Multipilication) / (double)CLOCKS_PER_SEC << " Sec" << endl;
        //cout << "\nStrassen mode " << (endTime_For_Strassen - startTime_For_Strassen) / (double)CLOCKS_PER_SEC << " Sec\n";
        //system("Pause");
        //a = a + time_1 / MatrixSize * 2;
        //cout << a << endl;
        //float c = MatrixSize * 194.784;
        //cout << c << endl;
        if (MatrixSize == 4) {
            int rows = 4;
            int col = 4;
            ...
            MUL(MatrixA, MatrixB, A, MatrixSize);
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < col; j++) {
                    cout << A[i][j] << '\t';
                }
...
void Strassen(int N, int** MatrixA, int** MatrixB, int** MatrixC)
{
    ...
    if (N <= 2)//choosing the threshhold is extremely important, try N<=2 to see the result
    {
...
void initialMatrix(int** MatrixA, int** MatrixB, int MatrixSize)
{
    for (int row = 0; row < MatrixSize; row++)
    {
        for (int column = 0; column < MatrixSize; column++)
        {
            ...
            MatrixB[row][column] = (MatrixA[row][column] = rand() % MatrixSize);
        }
